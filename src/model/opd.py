...
# prompt tuning
class OPD_Prompt(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        self.prompt_embedding_task = Embedding(
            vocab_size =  1 * config.prompt_length, 
            embedding_size = config.dim_model,
            length_scale = config.length_scale,
            dtype = config.dtype,
            int8 = config.int8,
    ...
    def forward(self, input : torch.Tensor, # (batch, seqlen)
        ...
        if past_key_values is None:
            ...
            offset = input_prompt[0][0].item()
            input_prompt = input_prompt - offset
            prompt_states = self.prompt_embedding_task(input_prompt)
            hidden_states = self.input_embedding(input_ids)
            hidden_states = torch.cat([prompt_states, hidden_states], 1)
