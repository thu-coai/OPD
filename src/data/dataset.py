...
class OPD_Dataset(data.Dataset):
    def __init__(self, ctx : MMapIndexedDataset, info: MMapIndexedDataset, 
                 max_length = 2048, prompt_length = 64, tokenizer=None):
        self.ctx = ctx
        self.info = info
    ...
    def __get_item_data(self, ctx, info):
        ...
        inp = np.arange((self.prompt_length+len_ctx), dtype = np.int64) + self.prompt_length * task + self.tokenizer.vocab_size
